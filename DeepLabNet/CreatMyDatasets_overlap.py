...
for i in range(r1, r2+1):
    ...
    patches = break_image_overlap(I, output_size)
    for patch_j in range(patches.shape[2]):
        filename = os.path.join(targetDir, f"{s}_{patch_j+1}.png")
        cv2.imwrite(filename, patches[:,:,patch_j])

# Process label images to extract border
...
# Now break the processed border images into overlapping patches
